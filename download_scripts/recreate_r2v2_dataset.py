    ...
    while video_data is not None:
        ...
        if len(frames) > 0:
            ...
            for frame, frame_ind in zip(frames, frame_inds):
                cv2.imwrite(os.path.join(dataset_dir, video_id[:2], f"{video_id}_{frame_ind:0{LEN_NUM_NAME}d}.jpg"), frame)
            if len(frames) > 0:
...
def download_dataset(urls: List[str], frames: List[List[int]], dataset_dir: str):
    with mp.Manager() as  manager:
        ...
        thread.start()
        successful_download = list(tqdm.tqdm(pool.imap(try_download_and_save_frames, parallel_args), total=len(parallel_args)))
        thread.join()
